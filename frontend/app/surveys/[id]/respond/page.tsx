...
// ─── AI follow-up generator (calls backend; falls back to pattern-based) ──────
async function generateFollowup(surveyId: string, question: string, answer: string): Promise<string> {
  try {
    const res = await fetch(`${API}/surveys/${surveyId}/followup`, {
      method: 'POST',
...
function ConversationalMode({ survey, questions }: { survey: Survey; questions: Question[] }) {
  ...
  const submitAnswer = async (value: string, numericValue?: number) => {
    ...
    if ((q.type === 'short_text' || q.type === 'long_text') && !pendingFollowup) {
      const followup = await generateFollowup(survey.id, q.title, value)
        ...
        <div style={{ borderTop: '1px solid var(--border)', padding: '14px 16px' }}>
          {done ? (
            <div style={{ textAlign: 'center', padding: '30px 10px' }}>
              ...
              <div style={{ fontSize: 10.5, color: 'var(--ai)', marginTop: 8 }}>Follow-up questions are generated by Claude AI.</div>
            </div>
